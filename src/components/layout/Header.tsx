...
const locationCategories = [
  ...
  'Strasbourg', 'Nantes', 'Montpellier', 'Rennes', 'Grenoble', 'Toulon', 'Angers'
];

const Header = () => {
  const isMobile = useIsMobile();
  const { getAllProperties, properties, searchProperties } = useProperties();
    ...
    <header className="bg-white border-b border-gray-200 sticky top-0 z-50 w-full">
          ...
          <div className="flex-1 max-w-2xl mx-auto px-0 sm:px-4 relative">
            ...
            <form onSubmit={handleSearch} className="relative flex items-center">
                  ...
                  <PopoverTrigger asChild>
                    ...
                    >
                      <Input 
                        ...
                        className="w-full pl-10 pr-12 py-2 rounded-full border border-gray-200 shadow-md hover:shadow-lg transition-all duration-300 focus:ring-2 focus:ring-primary/30 focus:border-primary"
                        value={searchTerm}
                        onChange={(e) => {
                          ...
                          filteredLocations.map((location, index) => (
                            ...
                            >
                              <CommandItem
                                ...
                                className="flex items-center rounded-lg px-3 py-2 hover:bg-primary/5 transition-colors duration-200"
                              >
              ...
              <AnimatePresence>
                {showFilters && (
                  <motion.div 
                    className="absolute top-full left-0 right-0 mt-2 bg-white rounded-lg shadow-lg p-4 z-40"
                    initial={{ opacity: 0, y: -10 }}
                    animate={{ opacity: 1, y: 0 }}
                    exit={{ opacity: 0, y: -10 }}
              ...
              <AnimatePresence>
                {selectedLocation && !showFilters && (
                  <motion.div 
                    className="absolute top-full mt-2 left-4 right-4 flex items-center"
      ...
      {isMobile && (
        <AnimatePresence>
          {showFilters && (
            <motion.div 
              className="fixed inset-x-0 top-16 bg-white p-4 border-b border-gray-200 shadow-md z-50"
      ...
      <AnimatePresence>
        {isMobile && selectedLocation && !showFilters && (
          <motion.div 
            className="fixed top-16 inset-x-0 bg-white border-b border-gray-200 py-2 px-4 z-40 flex items-center"
            initial={{ opacity: 0, y: -10 }}
            animate={{ opacity: 1, y: 0 }}
            exit={{ opacity: 0, y: -10 }}
            transition={{ duration: 0.2 }}
